...
export function createCompactPlayer (host, movie, quality) {
  ...
  host.innerHTML = `
    <div class="video-surface"><video controls playsinline preload="auto" aria-label="Видео"></video>
    <div class="full-timeline"><label class="player-timeline">Позиция фильма <input id="audio-seek" type="range" min="0" max="0" step="1" value="0" disabled></label><span id="audio-time">Длительность уточняется…</span><button id="player-fullscreen" type="button">Полный экран</button></div></div>
  ...
  host.querySelector('#player-fullscreen').onclick = async () => {
    try { if (document.fullscreenElement) await document.exitFullscreen(); else await host.querySelector('.video-surface').requestFullscreen() } catch { host.querySelector('#status').textContent = 'Полный экран недоступен в этом браузере' }
  }
  return controller
}
