  ...
  const sql = async () => {
    ...
    connection ||= import('@neondatabase/serverless').then(async ({ neon }) => {
      ...
      await db.transaction([
        db.query('CREATE TABLE IF NOT EXISTS public.torfilms_accounts (id INTEGER PRIMARY KEY CHECK (id=1), revision INTEGER NOT NULL, payload JSONB NOT NULL)'),
        db.query('REVOKE ALL ON public.torfilms_accounts FROM PUBLIC'),
        db.query(`DO $$ BEGIN
          IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='anonymous') THEN
            REVOKE ALL ON public.torfilms_accounts FROM anonymous;
          END IF;
          IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='authenticated') THEN
            REVOKE ALL ON public.torfilms_accounts FROM authenticated;
          END IF;
        END $$`),
        db.query('ALTER TABLE public.torfilms_accounts ENABLE ROW LEVEL SECURITY')
      ])
