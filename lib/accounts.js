...
export function accounts (store, library, { publicOnly = false } = {}) {
  ...
  return { requireUser, async handle (req, res, url) {
    ...
    try {
      ...
      if (publicOnly && url.pathname.startsWith('/admin/')) { json(403, { error: 'Управление доступно только в админке' }); return true }
      ...
      if (route === '/requests' && req.method === 'GET') {
        const state = await store.read()
        json(200, state.requests.filter(r => (!publicOnly && user.role !== 'user') || r.author === user.id).map(r => publicOnly ? { id: r.id, kind: r.kind, draft: { id: r.draft.id, title: r.draft.title }, status: r.status, createdAt: r.createdAt, note: r.note } : r)); return true
      }
      if (route === '/requests' && req.method === 'POST') {
        ...
        // Public cards omit revisions and torrent bytes; preserve these on the server.
        const draft = structuredClone(data.draft)
        if (publicOnly) {
          const previous = (await library.all()).find(m => m.id === draft.id)
          draft.revision = previous?.revision || 0
          draft.sources = (draft.sources || []).map(source => {
            const old = previous?.sources.find(s => s.id === source.id)
            return old ? { ...old, ...source, magnet: source.magnet || old.magnet, torrentBase64: source.torrentBase64 || (source.magnet && source.magnet !== old.magnet ? '' : old.torrentBase64) } : source
          })
        }
        const request = { id: randomUUID(), author: user.id, kind: data.kind, draft, status: 'pending', createdAt: new Date().toISOString() }
        ...
        json(201, publicOnly ? { id: request.id, status: request.status } : request); return true
      }
      const review = /^\/requests\/([a-f0-9-]+)\/review$/.exec(route)
      if (review && req.method === 'POST') {
        if (publicOnly) { json(403, { error: 'Обработка заявок доступна только в админке' }); return true }
