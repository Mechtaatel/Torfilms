...
export function catalog (directory) {
  ...
  async function describeSource ({ torrentBase64, ...source }) {
    ...
    for (const [index, file] of (parsed.files || []).entries()) {
      ...
      const externalTracks = externalAudioFiles(parsed.files, index)
      if (saved || externalTracks.length) audioMetadata[index] = saved || { tracks: [], externalTracks, pending: true, duration: 0 }
    }
    return { ...source, infoHash: parsed.infoHash, audioMetadata, hasTorrent: !!torrentBase64 }
  }
  ...
  async function normalize (input) {
    ...
    for (const source of input.sources || []) {
      ...
      sources.push({ id: /^[a-zA-Z0-9-]{1,40}$/.test(source.id || '') ? source.id : randomUUID(), label, season, magnet: `magnet:?xt=urn:btih:${parsed.infoHash}${params.size ? '&' + params : ''}`, torrentBase64, fileIndex: Math.max(0, Math.floor(Number(source.fileIndex) || 0)) })
    }
