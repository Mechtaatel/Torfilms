...
export function catalog (directory) {
  ...
  async function normalize (input) {
    ...
    for (const source of sources) {
      const approved = input.sources.find(s => s.id === source.id)?.publicPlayback?.infoHash
      if (!approved) continue
      const parsed = await parseTorrent(source.torrentBase64 ? Buffer.from(source.torrentBase64, 'base64') : source.magnet)
      if (parsed.private !== true && parsed.infoHash === approved) source.publicPlayback = { infoHash: approved }
    }
