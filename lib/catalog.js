  ...
  async function normalize (input) {
    ...
    for (const source of input.sources || []) {
      ...
      for (const e of source.episodes || []) {
        ...
        const episodeSeason = e.season == null ? season : Number(e.season)
        if (episodeSeason !== null && (!Number.isInteger(episodeSeason) || episodeSeason < 0 || episodeSeason > 999)) throw new Error('Некорректный сезон серии')
        if (episodeSeason !== null && !seasons.some(s => s.number === episodeSeason)) seasons.push({ number: episodeSeason, title: `Сезон ${episodeSeason}`, poster: '' })
