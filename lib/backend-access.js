...
export function backendAccess (req, res, env = process.env) {
  ...
  const publicAccountRoute = (req.method === 'GET' && pathname === '/auth/me') || (req.method === 'POST' && ['/auth/register', '/auth/login', '/auth/logout'].includes(pathname)) || (['GET', 'POST'].includes(req.method) && pathname === '/requests')
  if (env.TORFILMS_ADMIN_ENABLED !== '1' && !publicAccountRoute && (/^\/(auth|admin|requests)(\/|$)/.test(pathname) || (pathname.startsWith('/catalog/') && !['GET', 'HEAD'].includes(req.method)))) {
    res.writeHead(403, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ error: 'Управление отключено на сервере просмотра' })); return false
  }
