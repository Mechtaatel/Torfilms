...
export function createLanProxy (upstreamPort, { tls, websocket = false } = {}) {
  ...
  const handler = (req, res) => {
    ...
    if (req.headers.origin && req.headers.origin !== `${protocol}//${req.headers.host}`) return fail(403, 'Cross-origin request blocked')
  ...
  server.on('upgrade', (req, socket, head) => {
    if (!websocket || !/^\/tracker(?:\/[a-f0-9]{40})?$/.test(req.url) || (req.headers.origin && req.headers.origin !== `${protocol}//${req.headers.host}`)) return socket.destroy()
