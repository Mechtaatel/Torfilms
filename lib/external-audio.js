...
export function externalAudioFiles (files, videoIndex, subtitles = false) {
  ...
  return files.flatMap((file, fileIndex) => {
    if (!(subtitles ? /\.(ass|ssa|srt|vtt)$/i : /\.(mka|m4a|aac|ac3|eac3|dts|flac|ogg|opus|mp3|wav)$/i).test(basename(file))) return []
